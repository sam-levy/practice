<script lang="ts">
  ...
  onMount(() => {
    if (!initialized) {
      testSession.startTest(data.testId, data.testData);
      initialized = true;
    }
  });
...
<svelte:head>
  <title>{data.testData.test_title}</title>
</svelte:head>

<div class="container mx-auto p-4 pb-32 max-w-4xl">
  <h1 class="text-2xl font-bold mb-2">{data.testData.test_title}</h1>
  ...
  <div class="space-y-6">
    {#each data.testData.questions as question, i}
